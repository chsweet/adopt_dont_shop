...
RSpec.describe 'the shelters index' do
  ...
  it 'lists the shelters by most recently created first' do
    ...
    within "#shelter-#{@shelter_1.id}" do
      expect(page).to have_content("Created at: #{@shelter_1.created_at.strftime("%Y-%m-%d")}")
    end

    within "#shelter-#{@shelter_2.id}" do
      expect(page).to have_content("Created at: #{@shelter_2.created_at.strftime("%Y-%m-%d")}")
    end

    within "#shelter-#{@shelter_3.id}" do
      expect(page).to have_content("Created at: #{@shelter_3.created_at.strftime("%Y-%m-%d")}")
    end
  end
